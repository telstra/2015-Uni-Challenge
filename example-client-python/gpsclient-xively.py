...
"""
****Important**** 
As Telstra is not affiliated with Xively we can not provide you with any offical
support in using this platfrom or script. Please also note Xively imposes API read/write
limits and limits on the number of connected devices depending on the type account 
you have with them.
*****************

gpsclient-xively - this program takes GPS location fixes using a Netgear GPS enabled 
AirCard 760S modem and posts the GPS lat and long to the Xively cloud platfrom.

This script will not work without first creating an account with Xively (xiverly.com)
If you choose to use this script it is recomended to first go through the Xively 
tutorial (xively.com/testdrive) to better understand how this platfrom works.

Before running the script please change the following global variables:
1) The team name listed in the global variables
2) The Xively FeedID
3) The Xively ApiKey
...
FeedID = "000000000" # Set to your own personal device Xively Feed ID. To get ID register with xively.com and add device
XivelyApiKey = '000000000000000000000000000000000000000000000000' #Set to your own personal device Xively API Key.

DELAY = 10 # Seconds. Delay after JSON post.

API_Endpoint = "https://api.xively.com/v2/feeds/" + FeedID + ".json"
M2MserverUrl = API_Endpoint  # Xively URL
startTime = 0
...
def postJsonM2MServer(targetUrl, latitude, longitude, timedateUTC, imei, imsi, cpuID, displayString):
    """Posts data via Json Put to Xively cloud server"""
    print "[ -- ] Posting data to Xively", targetUrl
    print "       Name:", str(displayString), " Time:", str(timedateUTC)
    print "       cpuID:", str(cpuID)
    print "       Posting GPS coordinates lat:", str(latitude), "and long:", str(longitude)
    postdata = json.dumps({
        "version" : "1.0.0",
        "datastreams" : [
            {"id" : "Name", "current_value" : str(displayString)},
            {"id" : "Time", "current_value" : str(timedateUTC)},
            {"id" : "cpuID", "current_value" : str(cpuID)},
            {"id" : "latitude", "current_value" : str(latitude)},
            {"id" : "logitude", "current_value" : str(longitude)}
        ],
        "location" : {'lat' : str(latitude),'lon' : str(longitude)}
    })
    headers = {"X-ApiKey": XivelyApiKey}
        ...
        if currentState == "GetCPUid":
            ...
            if matchcommand and response:
                ...
                if currentState == "GetGPSstatus":
                    ...
                    #print "[TEST] Posting test location Uluru -25.345 and 131.035 using time 2013-01-01 00:00:00"
                    #imei = statedictionary['GetIMEI']['value']
                    #imsi = statedictionary["GetIMSI"]['value']
                    #cpuID = statedictionary["GetCPUid"]['value']
